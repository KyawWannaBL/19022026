...
function cdnPrefixImages(): Plugin {
  ...
  const rewriteHtml = (html: string, cdn: string) => {
    html = html.replace(/(src|href)\s*=\s*(['"])([^'"]+)\2/g, (_m, k, q, p) => `${k}=${q}${toCDN(p, cdn)}${q}`);
    html = html.replace(/(srcset)\s*=\s*(['"])([^'"]+)\2/g, (_m, k, q, list) => `${k}=${q}${rewriteSrcsetList(list, cdn)}${q}`);
    return html;
  };
  ...
  const rewriteJsxAst = (code: string, id: string, cdn: string) => {
    ...
    traverse(ast, {
      ...
      StringLiteral(path) {
        ...
        if (t.isImportDeclaration(path.parent) || t.isExportAllDeclaration(path.parent)) return;
    ...
    return rewrites ? generate(ast, { retainLines: true }, code).code : null;
  };
  ...
  return {
    ...
    apply: 'build',
    enforce: 'pre',
    configResolved(cfg) { publicDir = cfg.publicDir; },
    async buildStart() { await collectPublicImagesFrom(publicDir); },
    transformIndexHtml(html) {
      const cdn = process.env.CDN_IMG_PREFIX;
      return cdn ? rewriteHtml(html, cdn) : html;
    },
    transform(code, id) {
      const cdn = process.env.CDN_IMG_PREFIX;
      if (!cdn) return null;
