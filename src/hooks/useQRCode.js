...
export function useQRCode() {
     ...
     */
    const getShipmentPayload = useCallback((shipment) => {
     ...
     */
    const trackActivity = useCallback(async (shipmentId, action, metadata) => {
     ...
     */
    const downloadQRCode = useCallback((canvasId, fileName) => {
     ...
     */
    const printQRCode = useCallback((canvasId, label) => {
        const canvas = document.getElementById(canvasId);
        if (!canvas) {
            setError('QR Canvas element not found');
            return;
