...
export default function WarehouseSorting() {
    ...
    const loadData = async () => {
        try {
            ...
            const todaySorted = operations.filter(op => new Date(op.created_at).toDateString() === today &&
    ...
    const filterParcels = () => {
        ...
        // Search filter
        if (searchTerm) {
            filtered = filtered.filter(p => p.tracking_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
