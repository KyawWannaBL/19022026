    ...
    const filteredShipments = useMemo(() => {
        return mockShipments.filter((shipment) => {
            return shipments.filter((shipment) => {
                const matchesSearch = shipment.awb.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        const columns = [
            {
                ...
                accessorKey: 'awb',
                cell: (item) => (_jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: "font-mono font-bold text-primary", children: item.awb }), _jsxs("span", { className: "text-xs text-muted-foreground flex items-center gap-1", children: [_jsx(Calendar, { className: "w-3 h-3" }), new Date(item.createdAt).toLocaleDateString()] })] })),
            },
            {
                header: 'Sender',
