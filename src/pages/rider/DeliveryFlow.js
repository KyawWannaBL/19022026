...
export default function DeliveryFlow() {
  ...
  const { t } = useLanguageContext();
    ...
    const receiverNameNameRef = useRef(null);
    ...
    const canSubmitDelivery = useMemo(() => {
        if (!podData.receiverNameName || !podData.signature)
            return false;
    ...
    useEffect(() => {
        if (step === 'pod') {
            setTimeout(() => receiverNameNameRef.current?.focus(), 50);
        }
    }, [step]);
    ...
    const handleScan = useCallback((code) => {
        ...
        setOtpValue('');
        setPodData({ receiverNameName: '', relationship: '', signature: '', photo: '' });
    ...
    const handleDeliver = useCallback(async () => {
        ...
        try {
            ...
            const nonRepudiablePODPayload = {
                ...
                awb: shipment.awb,
                riderId,
                receiverNameName: podData.receiverNameName,
