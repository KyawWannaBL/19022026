...
export const LanguageToggle: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  return (
    <button
      ...
      className={`language-toggle group ${className}`}
...
export const CompactLanguageToggle: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  return (
    <button
      onClick={toggleLanguage}
      className={`relative p-2 rounded-lg bg-navy-800/80 border border-gold-400/30 text-gold-400 hover:text-gold-300 transition-all duration-300 hover:shadow-lg hover:shadow-gold-500/20 ${className}`}
      title={language === 'en' ? 'Myanmar' : 'English'}
    >
