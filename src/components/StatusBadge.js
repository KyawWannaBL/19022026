...
import { CheckCircle2, Clock, AlertTriangle, Truck, Package, FileText, Printer, MapPin, UserCheck, XCircle, ShieldAlert, Archive, PackageCheck } from 'lucide-react';
import { SHIPMENT_STATUS, TAG_STATUS } from '@/lib/index';
function cn(...inputs) {
...
export default function StatusBadge({ status, size = 'md' }) {
    ...
    return (_jsxs("div", { className: cn('inline-flex items-center font-medium rounded-full border transition-colors', config.color, sizeClasses[size]), children: [_jsx(Icon, { size: iconSizes[size], className: "shrink-0" }), _jsx("span", { className: "whitespace-nowrap", children: config.label })] }));
}
