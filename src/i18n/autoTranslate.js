...
const elemOriginalAttrs = new WeakMap();
function isCodeLike(text) {
    ...
    if (digits > 0 && letters > 0 && t.length <= 12)
        return true;
    return false;
}
...
function translateTextNode(node, lang) {
    ...
    if (!nodeOriginalText.has(node))
        nodeOriginalText.set(node, raw);
    const original = nodeOriginalText.get(node) ?? raw;
    if (isCodeLike(original))
        return;
...
export function applyAutoTranslate({ root, lang }) {
    // translate existing
...
export function observeAutoTranslate({ root, lang }) {
            ...
            if (m.type === "characterData" && m.target) {
                ...
                m.addedNodes.forEach((node) => {
                    if (node.nodeType === Node.TEXT_NODE) {
                        ...
                        applyAutoTranslate({ root: node, lang });
                    }
                });
            }
